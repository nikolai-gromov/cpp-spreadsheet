...
public:
    ...
    virtual double Evaluate(std::function<std::variant<double, FormulaError>(Position)> args) const = 0;
...
public:
   ...
   double Evaluate(std::function<std::variant<double, FormulaError>(Position)> args) const override {
        ...
        switch (type_) {
            ...
            case Multiply:
                return lhs_->Evaluate(args) * rhs_->Evaluate(args);
            case Divide:
                if (std::abs(rhs_->Evaluate(args)) < epsilon) {
...
public:
    ...
    double Evaluate(std::function<std::variant<double, FormulaError>(Position)> args) const override {
        switch (type_) {
            ...
            case UnaryMinus:
                return operand_->Evaluate(args) * -1.0;
            default:
                throw FormulaError(FormulaError::Category::Value);
        }
    }
...
public:
    ...
    double Evaluate(std::function<std::variant<double, FormulaError>(Position)> args) const override {
        try {
            std::variant<double, FormulaError> value = args(*cell_);
...
public:
    ...
    double Evaluate(std::function<std::variant<double, FormulaError>(Position)> args) const override {
        const double max_lower_bound = 1.79769e+308;
        const double max_upper_bound = std::numeric_limits<double>::max();
        if (value_ >= max_lower_bound && value_ <= max_upper_bound) {
            throw FormulaError(FormulaError::Category::Div0);
        }
        return value_;
    }
...
double FormulaAST::Execute(std::function<std::variant<double, FormulaError>(Position)> args) const {
